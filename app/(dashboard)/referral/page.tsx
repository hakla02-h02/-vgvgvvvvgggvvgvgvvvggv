  ...
  return (
    <>
      ...
      <ScrollArea className="flex-1">
        <div className="min-h-full bg-background text-foreground pb-8">
          <div className="w-full max-w-md mx-auto px-4 sm:px-6 lg:max-w-2xl space-y-6 pt-6">
            ...
            <section className="text-center space-y-2">
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground text-balance">
                Convide amigos e ganhe <span className="text-accent">comissoes</span>
              </h2>
            ...
            <div className="space-y-3">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Seu link de indicacao</label>
              ...
              {coupon ? (
                <>
                  {isEditing ? (
                    <div className="flex flex-col gap-3">
                      <div className="flex gap-2">
                        <Input
                          ...
                          className="flex-1 bg-card border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:border-accent h-12"
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              e.preventDefault()
                              handleUpdateCoupon()
                        ...
                        <button
                          ...
                          className="bg-card border border-border p-3 rounded-xl flex items-center justify-center disabled:opacity-50 hover:bg-muted transition-colors"
                        >
                          {isUpdating ? <Loader2 className="h-5 w-5 animate-spin text-foreground" /> : (
                        ...
                        <button
                          ...
                          disabled={isUpdating}
                          className="bg-card border border-border p-3 rounded-xl flex items-center justify-center hover:bg-muted transition-colors"
                        >
                    ...
                    <>
                      <div className="flex gap-2">
                        <div className="flex-1 min-w-0 bg-card border border-border rounded-xl px-4 flex items-center h-12">
                          <span className="text-muted-foreground text-sm truncate">{referralLink}</span>
                        </div>
                        <button
                          onClick={handleCopy}
                          className="flex-shrink-0 bg-card border border-border p-3 rounded-xl flex items-center justify-center hover:bg-muted transition-colors"
                        >
                          {copied ? (
                            <svg viewBox="0 0 24 24" className="w-5 h-5 text-accent" fill="none" stroke="currentColor" strokeWidth="2">
                              <polyline points="20 6 9 17 4 12"/>
                ...
                <div className="flex flex-col gap-3">
                  <Input
                    ...
                    className="w-full bg-card border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:border-accent h-12"
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault()
                        handleCreateCoupon()
                  ...
                  <button 
                    ...
                    className="w-full bg-accent hover:bg-accent/90 text-accent-foreground font-bold py-3 rounded-xl flex items-center justify-center gap-2 disabled:opacity-50 transition-colors"
                  >
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-card border border-border p-4 rounded-xl space-y-3">
                  ...
                  <p className="text-sm font-semibold leading-tight text-foreground">Compartilhe seu link exclusivo</p>
                </div>
                <div className="bg-card border border-border p-4 rounded-xl space-y-3">
                  <div className="w-10 h-10 rounded-full bg-warning/10 flex items-center justify-center">
                    <svg viewBox="0 0 24 24" className="w-5 h-5 text-warning" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <p className="text-sm font-semibold leading-tight text-foreground">Seu amigo se cadastra e assina</p>
                </div>
                <div className="bg-card border border-border p-4 rounded-xl space-y-3">
                  <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center">
                    <svg viewBox="0 0 24 24" className="w-5 h-5 text-accent" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <p className="text-sm font-semibold leading-tight text-foreground">Voce recebe sua comissao</p>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Indicacoes Recentes</h3>
                {referrals.length > 0 && (
                  <button className="text-xs text-accent font-medium hover:text-accent/80 transition-colors">Ver todas</button>
                )}
              </div>
              ...
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                {referrals.length === 0 ? (
                  <div className="flex flex-col items-center gap-4 py-10 text-center px-6">
                    ...
                    <div>
                      <p className="font-semibold text-foreground">Nenhum indicado ainda</p>
                      <p className="text-xs text-muted-foreground mt-1">Compartilhe seu link e veja seus indicados aqui</p>
                    </div>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left text-xs min-w-[300px]">
                      <thead>
                        <tr className="border-b border-border bg-muted/50">
                          ...
                          <th className="px-4 py-3 font-semibold text-muted-foreground text-right">Comissao</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-border">
                        {referrals.slice(0, 5).map((ref) => (
                          <tr key={ref.id} onClick={() => setSelectedUser(ref)} className="cursor-pointer hover:bg-muted/50 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex flex-col">
                                <span className="font-semibold text-foreground">{ref.name}</span>
                                <span className="text-[10px] text-muted-foreground">{formatDate(ref.referral_date)}</span>
                              </div>
                            </td>
                            <td className="px-4 py-3">
                              <span className={cn(
                            ...
                            <td className="px-4 py-3 text-right">
                              <span className="font-semibold text-foreground">R$ 0,10</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {/* CTA AI Card */}
            <div className="relative overflow-hidden rounded-xl p-5 bg-gradient-to-r from-warning/10 to-card border border-warning/20 flex items-center gap-4">
              <div className="relative w-14 h-14 flex-shrink-0">
                <div className="absolute inset-0 bg-warning/20 rounded-full blur-lg"></div>
                <div className="relative w-full h-full rounded-full bg-gradient-to-tr from-warning to-warning/80 flex items-center justify-center shadow-lg">
                  <svg viewBox="0 0 24 24" className="w-7 h-7 text-warning-foreground" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="space-y-1 min-w-0">
                <h4 className="text-foreground font-bold leading-tight">Escale seus ganhos</h4>
                <p className="text-xs text-muted-foreground">Torne-se um embaixador e tenha beneficios exclusivos de IA.</p>
              </div>
            </div>
      ...
      <Dialog open={!!selectedUser} onOpenChange={() => setSelectedUser(null)}>
        <DialogContent className="bg-card border-border text-foreground rounded-xl max-w-md">
          <DialogHeader>
          ...
          {selectedUser && (
            <div className="space-y-4 pt-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-accent to-accent/70 flex items-center justify-center text-accent-foreground font-bold text-xl">
                  {selectedUser.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="font-semibold text-lg text-foreground">{selectedUser.name}</p>
                  <div className={cn(
              ...
              <div className="space-y-3 bg-muted/50 rounded-xl p-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-muted-foreground">{selectedUser.phone || "Nao informado"}</span>
                </div>
                <div className="flex items-center gap-3">
                  <svg viewBox="0 0 24 24" className="w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <span className="text-sm text-muted-foreground">Indicado em {formatDate(selectedUser.referral_date)}</span>
                </div>
              </div>
            </div>
          )}
